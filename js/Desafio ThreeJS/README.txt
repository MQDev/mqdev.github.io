 ...
 // in the animation loop
  ...
  function preencheArrayElementosTeste(arrayElements,numObjects){
        for (let i = 0; i < numObjects; ++i) {
            ...
            for(let j=0;j<8;j++){
                if(j<3){
                    arrayElements[i,j]=rand(-20, 20);
                } else if(j<5){
                    arrayElements[i,j]=rand(Math.PI);
                } else if (j=5){
                    arrayElements[i,j]=0;
                } else {
                    arrayElements[i,j]=rand(3, 6);
                }
            }
        }
    }
    ...
    posicionaElementosXML(numObjects,arrayElements);

    function posicionaElementosXML(numObjects, arrayElements){
        const idToObject = {};
        ...
        for (let i = 0; i < numObjects; ++i) {
            ...
            cube.position.set(arrayElements[i,0],arrayElements[i,1],arrayElements[i,2]);
            cube.rotation.set(arrayElements[i,3],arrayElements[i,4],0);
            cube.scale.set(arrayElements[i,5],arrayElements[i,6],arrayElements[i,7]);
